...
def calcular_centro_zoom(lats, lons, padding=1.2):
    """
    Calcula centro e nível de zoom do mapa a partir da dispersão real dos pontos.
    Filtra coordenadas inválidas (0,0) e ajusta zoom para rotas pequenas.
    """
    # Filtra coordenadas válidas (lat/lon diferentes de 0)
    validos = [(lat, lon) for lat, lon in zip(lats, lons) 
               if abs(lat) > 0.1 and abs(lon) > 0.1]
    
    if not validos:
        # Fallback: centro do Brasil
        return -14.235, -51.925, 4
    
    lats_validos = [lat for lat, lon in validos]
    lons_validos = [lon for lat, lon in validos]
    
    lat_min, lat_max = min(lats_validos), max(lats_validos)
    lon_min, lon_max = min(lons_validos), max(lons_validos)
    
    # Se todos os pontos forem iguais (ex: só CD), adiciona um padding
    ...
    # Calcula o range com padding
    lat_range = (lat_max - lat_min) * padding
    lon_range = (lon_max - lon_min) * padding
    max_range = max(lat_range, lon_range, 0.01)
    
    # Fórmula do zoom: quanto menor a área, maior o zoom
    # Ajustado para não ficar nem muito perto nem muito longe
    zoom = math.log2(360 / max_range) - 1
    
    # Limita o zoom entre 8 e 16 (cidade/rua)
